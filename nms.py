...
def nm_s(bboxes, scores):
    ...
    for i in range(0, len(sorted_idx)):
        ...
        center = [curr_bbox[0] + ((curr_bbox[2] - curr_bbox[0]) / 2),
                  curr_bbox[1] + ((curr_bbox[3] - curr_bbox[1]) / 2)]
        for j in range(i + 1, len(sorted_idx)):
            if is_valid_bbox[sorted_idx[j]] != 0:
                my_bbox = bboxes[sorted_idx[j]]
                if center[0] >= my_bbox[0] and center[0] <= my_bbox[2] and center[1] >= my_bbox[1] and center[1] <= \
                        my_bbox[3]:
